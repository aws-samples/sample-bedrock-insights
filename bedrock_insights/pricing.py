...
def _fetch_live_csv(region: str) -> tuple[dict, dict]:
    ...
    try:
        ...
        # URL is an AWS-issued presigned S3 link; still confirm it's http(s)
        # before opening it (rejects file://, ftp:// etc.) as defense in depth.
        if not url.lower().startswith(("http://", "https://")):
            return {}, {}
        # nosemgrep: dynamic-urllib-use-detected — AWS-issued URL, scheme validated to http(s) above
